...
latitude value (lat) and a longitude value (lng) (these 
are GPS coordinates, examples are below).

2. Do 'reverse geocoding' of the provided coordinates. 
Reverse geocoding means to convert coordinates to a 
meaningful location, like a city and country name. Use this 
...
const whereAmI = function (lat,lng) {
  ...
  .then(res => {
    if (!res.ok) throw new Error(`country not found (${data.status}) :(`)
...
whereAmI(10.8312478,78.6995237);
...
const lotteryPromise = new Promise(function (resolve,reject) {
  console.log('Lottery Result Happening! 🔮')
  setTimeout(() =>{
    if (Math.random() >= 0.5){
    resolve('You are Win! 💰');
  } else {
    reject(new Error('You are lost your money 💩'));
  }
  },2000)
})
...
const getPosition = function () {
  return new Promise(function (resolve,reject) {
    ...
    // .getCurrentPosition(position => resolve(position),
    // err => reject(err)
    // );
...
figure out some stuff on your own. Pretend you're working on 
your own 😉

PART 1
1. Create a function 'createImage' which receives imgPath as 
an input. This function returns a promise which creates a new
image (use document.createElement('img')) and sets the .src
attribute to the provided image path. When the image is done
loading, append it to the DOM element with the 'images' 
class, and resolve the promise. The fulfilled value should 
be the image element itself. In case there is an error 
loading the image ('error' event), reject the promise.

If this part is too tricky for you, just watch the first 
part of the solution.

PART 2
2. Comsume the promise using .then and also add an error 
handler;
...
5. After the second image has loaded, pause execution for 2 
seconds again;

6. After the 2 seconds have passed, hide the current image.

TEST DATA: Images in the img folder. Test the error handler 
by passing a wrong image path. Set the network speed to 
'Fast 3G' in the dev tools Network tab, otherwise images 
load too fast.
...
const whereAmI = async function () {
  ...
  try {
    ...
    const {latitude : lat,longitude : lng} = pos.coords;
    // console.log(lat,lng) // current location
...
getCountries('germany','russia','india');
...
Challenge #2, this time using async/await (only the part 
where the promise is consumed). Compare the two versions, 
think about the big differences, and see which one you 
like more.

Don't forget to test the error handler, and to set the 
network speed to 'Fast 3G' in the dev tools Network tab.

PART 2
1. Create an async function 'loadAll' that receives an 
array of image paths 'imgArr';

2. Use .map to loop over the array, to load all the images 
with the 'createImage' function (call the resulting array 
'imgs')

3. Check out the 'imgs' array in the console! Is it like 
you expected?
